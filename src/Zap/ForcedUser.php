...
class ForcedUser {
	 ...
	 */
	public function isForcedUserModeEnabled($apikey='') {
		return $this->zap->request($this->zap->base . 'forcedUser/view/isForcedUserModeEnabled/', array('apikey' => $apikey))->{'isForcedUserModeEnabled'};
	}

	/**
	 * Gets the user (ID) set as 'forced user' for the given context (ID)
	 */
	public function getForcedUser($contextid, $apikey='') {
		return $this->zap->request($this->zap->base . 'forcedUser/view/getForcedUser/', array('contextId' => $contextid, 'apikey' => $apikey))->{'getForcedUser'};
	}
