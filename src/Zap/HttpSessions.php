...
class HttpSessions {
	...
	public function sites($apikey='') {
		return $this->zap->request($this->zap->base . 'httpSessions/view/sites/', array('apikey' => $apikey))->{'sites'};
	}

	/**
	 * Gets the sessions for the given site. Optionally returning just the session with the given name.
	 */
	public function sessions($site, $session=NULL, $apikey='') {
	 ...
	 */
	public function activeSession($site, $apikey='') {
		return $this->zap->request($this->zap->base . 'httpSessions/view/activeSession/', array('site' => $site, 'apikey' => $apikey))->{'activeSession'};
	}

	/**
	 * Gets the names of the session tokens for the given site.
	 */
	public function sessionTokens($site, $apikey='') {
		return $this->zap->request($this->zap->base . 'httpSessions/view/sessionTokens/', array('site' => $site, 'apikey' => $apikey))->{'sessionTokens'};
	}

	/**
	 * Gets the default session tokens.
	 */
	public function defaultSessionTokens($apikey='') {
		return $this->zap->request($this->zap->base . 'httpSessions/view/defaultSessionTokens/', array('apikey' => $apikey))->{'defaultSessionTokens'};
	}
	 ...
	 */
	public function addDefaultSessionToken($sessiontoken, $tokenenabled=NULL, $apikey='') {
		$params = array('sessionToken' => $sessiontoken, 'apikey' => $apikey);
	 ...
	 */
	public function setDefaultSessionTokenEnabled($sessiontoken, $tokenenabled, $apikey='') {
		return $this->zap->request($this->zap->base . 'httpSessions/action/setDefaultSessionTokenEnabled/', array('sessionToken' => $sessiontoken, 'tokenEnabled' => $tokenenabled, 'apikey' => $apikey));
	}

	/**
	 * Removes the default session token with the given name.
	 */
	public function removeDefaultSessionToken($sessiontoken, $apikey='') {
		return $this->zap->request($this->zap->base . 'httpSessions/action/removeDefaultSessionToken/', array('sessionToken' => $sessiontoken, 'apikey' => $apikey));
	}
